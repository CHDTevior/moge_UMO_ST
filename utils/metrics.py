...
def calculate_pa_mpjpe(gt_joints, pred_joints, eps=1e-8):
    """
    Procrustes-aligned MPJPE per pose.

    gt_joints: num_poses x num_joints x 3
    pred_joints: num_poses x num_joints x 3
    """
    assert gt_joints.shape == pred_joints.shape, f"GT shape: {gt_joints.shape}, pred shape: {pred_joints.shape}"
    if gt_joints.ndim != 3 or gt_joints.shape[-1] != 3:
        raise ValueError(f"Expected [N,J,3] joints, got {tuple(gt_joints.shape)}")

    gt = gt_joints.float()
    pred = pred_joints.float()
    mu_gt = gt.mean(dim=1, keepdim=True)
    mu_pred = pred.mean(dim=1, keepdim=True)
    gt0 = gt - mu_gt
    pred0 = pred - mu_pred

    norm_gt = torch.linalg.norm(gt0.reshape(gt0.shape[0], -1), dim=1).clamp_min(float(eps))
    norm_pred = torch.linalg.norm(pred0.reshape(pred0.shape[0], -1), dim=1).clamp_min(float(eps))
    gt0_norm = gt0 / norm_gt[:, None, None]
    pred0_norm = pred0 / norm_pred[:, None, None]

    h = pred0_norm.transpose(1, 2) @ gt0_norm
    u, s, vh = torch.linalg.svd(h)
    v = vh.transpose(-2, -1)

    z = torch.eye(3, device=gt.device, dtype=gt.dtype).unsqueeze(0).repeat(gt.shape[0], 1, 1)
    det = torch.linalg.det(u @ v.transpose(-2, -1))
    z[:, -1, -1] = torch.where(det < 0, -torch.ones_like(det), torch.ones_like(det))

    rotation = u @ z @ v.transpose(-2, -1)
    trace = (s * torch.diagonal(z, dim1=-2, dim2=-1)).sum(dim=1)
    scale = trace * norm_gt / norm_pred
    aligned = scale[:, None, None] * (pred0 @ rotation) + mu_gt
    return torch.linalg.norm(aligned - gt, dim=-1).mean(dim=-1)
