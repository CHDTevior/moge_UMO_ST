...
TEXT_FUSION_SPECS = {
    "f00": ("shared", "bidirectional"),
    "f10": ("shared", "asymmetric"),
    "f01": ("separate", "bidirectional"),
    "f11": ("separate", "asymmetric"),
}
TEXT_FUSION_MODES = tuple(TEXT_FUSION_SPECS)


def resolve_text_fusion_mode(mode: str) -> Tuple[str, str]:
    normalized = str(mode).lower()
    if normalized not in TEXT_FUSION_SPECS:
        raise ValueError(
            f"text_fusion_mode must be one of {TEXT_FUSION_MODES}, got {mode!r}"
        )
    return TEXT_FUSION_SPECS[normalized]
...
class MultiHeadAttention(nn.Module):
    ...
    def project_query(
        ...
        *,
        ...
        q = self.q_norm(q)
        if query_pos is not None and rope_axes_dims is not None:
            q_cos, q_sin = _rope_cos_sin(
                query_pos, self.head_dim, rope_axes_dims
            )
    ...
    def project_key_value(
        ...
        *,
        ...
        bsz, memory_len, _ = memory.shape
        k, v = self.kv(memory).chunk(2, dim=-1)
        k = k.view(
            bsz, memory_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        v = v.view(
            bsz, memory_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        k = self.k_norm(k)
        if memory_pos is not None and rope_axes_dims is not None:
            k_cos, k_sin = _rope_cos_sin(
                memory_pos, self.head_dim, rope_axes_dims
            )
            k = _apply_rope(k, k_cos, k_sin)
        return k, v

    def project_extra_key_value(
        self,
        extra_k: torch.Tensor,
        extra_v: torch.Tensor,
        *,
        ...
        if extra_k.shape != extra_v.shape:
            raise ValueError(
                "extra_k/extra_v shape mismatch: "
                f"{tuple(extra_k.shape)} vs {tuple(extra_v.shape)}"
            )
        if (
            extra_k.ndim != 3
            or extra_k.shape[2] != self.hidden_size
        ):
            raise ValueError(
                f"Expected extra_k/extra_v [B,T,{self.hidden_size}], "
                f"got {tuple(extra_k.shape)}"
            )
        bsz, extra_len, _ = extra_k.shape
        k = extra_k.view(
            bsz, extra_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        v = extra_v.view(
            bsz, extra_len, self.num_heads, self.head_dim
        ).transpose(1, 2)
        ...
        if extra_pos is not None and rope_axes_dims is not None:
            k_cos, k_sin = _rope_cos_sin(
                extra_pos, self.head_dim, rope_axes_dims
            )
            k = _apply_rope(k, k_cos, k_sin)
        return k, v
    ...
    def project_output(
        ...
        *,
        ...
        bsz, _, query_len, _ = out.shape
        out = out.transpose(1, 2).contiguous().view(
            bsz, query_len, self.hidden_size
        )
        out = self.out(out)
    ...
    def forward(
        ...
        q = self.project_query(
            query,
            query_pos=query_pos,
            rope_axes_dims=rope_axes_dims,
        )
        k, v = self.project_key_value(
            memory,
        ...
        return self.project_output(out, query_valid=query_valid)
...
class DoubleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        projection_mode, attention_mode = resolve_text_fusion_mode(
            text_fusion_mode
        )
        ...
        canonical_attn = MultiHeadAttention(hidden_size, num_heads, dropout)
        if self.projection_mode == "shared":
            self.joint_attn = canonical_attn
            self.motion_attn = None
            self.text_attn = None
        else:
            self.joint_attn = None
            self.motion_attn = canonical_attn
            # Start the two modalities from the exact shared-attention solution
            # without consuming extra RNG; they remain independent parameters.
            self.text_attn = copy.deepcopy(canonical_attn)
    ...
    def _shared_attention(
        ...
        *,
        ...
        control_k: Optional[torch.Tensor],
        control_v: Optional[torch.Tensor],
        control_valid: Optional[torch.Tensor],
        control_pos: Optional[torch.Tensor],
        control_attn_bias: Optional[torch.Tensor],
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.joint_attn is None:
            raise RuntimeError("Shared attention is not initialized")
        ...
        if self.attention_mode == "bidirectional":
            ...
            return joint_out.split(
                [motion_in.shape[1], text_in.shape[1]], dim=1
            )

        motion_out = self.joint_attn(
            motion_in,
            joint,
            key_valid=joint_valid,
        ...
        text_out = self.joint_attn(
            text_in,
            text_in,
            key_valid=text_valid,
            query_valid=text_valid,
            query_pos=text_pos,
            memory_pos=text_pos,
            rope_axes_dims=rope_axes_dims,
        )
    ...
    def _separate_attention(
        ...
        *,
        ...
        if self.motion_attn is None or self.text_attn is None:
            raise RuntimeError("Separate stream attention is not initialized")
        motion_q = self.motion_attn.project_query(
            motion_in,
            query_pos=pos_ids,
            rope_axes_dims=rope_axes_dims,
        )
        motion_k, motion_v = self.motion_attn.project_key_value(
            motion_in,
            memory_pos=pos_ids,
            rope_axes_dims=rope_axes_dims,
        )
        text_q = self.text_attn.project_query(
            text_in,
            query_pos=text_pos,
            rope_axes_dims=rope_axes_dims,
        )
        text_k, text_v = self.text_attn.project_key_value(
            text_in,
            memory_pos=text_pos,
            rope_axes_dims=rope_axes_dims,
        )

        motion_memory_k = torch.cat([motion_k, text_k], dim=2)
        motion_memory_v = torch.cat([motion_v, text_v], dim=2)
        motion_memory_valid = torch.cat(
            [motion_valid, text_valid], dim=1
        )
        extra_key_count = 0
        if control_k is not None or control_v is not None:
            if control_k is None or control_v is None:
                raise ValueError(
                    "control_k and control_v must be provided together"
                )
            extra_k, extra_v = self.motion_attn.project_extra_key_value(
                control_k,
                control_v,
                extra_pos=control_pos,
                rope_axes_dims=rope_axes_dims,
            )
            extra_key_count = int(control_k.shape[1])
            motion_memory_k = torch.cat(
                [motion_memory_k, extra_k], dim=2
            )
            motion_memory_v = torch.cat(
                [motion_memory_v, extra_v], dim=2
            )
            if control_valid is None:
                control_valid = torch.ones(
                    control_k.shape[0],
                    control_k.shape[1],
                    device=control_k.device,
                    dtype=torch.bool,
                )
            motion_memory_valid = torch.cat(
                [
                    motion_memory_valid,
                    control_valid.to(
                        device=motion_memory_valid.device,
                        dtype=torch.bool,
                    ),
                ],
                dim=1,
        ...
        dropout_p = (
            self.motion_attn.dropout if self.training else 0.0
        )
        if self.attention_mode == "bidirectional":
            query = torch.cat([motion_q, text_q], dim=2)
            key = motion_memory_k
            value = motion_memory_v
            ...
            output = _attention(
                ...
                key,
                value,
                key_valid=motion_memory_valid,
                dropout_p=dropout_p,
                extra_key_count=extra_key_count,
                extra_attn_bias=control_attn_bias,
            )
            motion_heads, text_heads = output.split(
                [motion_in.shape[1], text_in.shape[1]], dim=2
            )
            motion_out = self.motion_attn.project_output(
                motion_heads, query_valid=motion_valid
            )
            text_out = self.text_attn.project_output(
                text_heads, query_valid=text_valid
            )
            return motion_out, text_out

        motion_heads = _attention(
            motion_q,
            motion_memory_k,
            motion_memory_v,
            key_valid=motion_memory_valid,
        ...
        text_heads = _attention(
            text_q,
            text_k,
            text_v,
            key_valid=text_valid,
            dropout_p=(
                self.text_attn.dropout if self.training else 0.0
            ),
        )
        return (
            self.motion_attn.project_output(
                motion_heads, query_valid=motion_valid
            ),
            self.text_attn.project_output(
                text_heads, query_valid=text_valid
            ),
        )
    ...
    def forward(
        ...
        text_pos = self._text_positions(text, pos_ids)
        attention_fn = (
            self._shared_attention
            if self.projection_mode == "shared"
            else self._separate_attention
        )
        motion_out, text_out = attention_fn(
            motion_in=motion_in,
            text_in=text_in,
            ...
            control_attn_bias=control_attn_bias,
        )
...
class SingleStreamBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        mlp_hidden = int(hidden_size * mlp_ratio)
        _, attention_mode = resolve_text_fusion_mode(text_fusion_mode)
        self.text_fusion_mode = str(text_fusion_mode).lower()
        self.attention_mode = attention_mode
    ...
    def forward(
        ...
        control_pos: Optional[torch.Tensor] = None,
        control_attn_bias: Optional[torch.Tensor] = None,
        motion_token_count: Optional[int] = None,
    ) -> torch.Tensor:
        ...
        if self.attention_mode == "bidirectional":
            ...
            if motion_token_count is None:
                raise ValueError(
                    "motion_token_count is required for asymmetric attention"
                )
            motion_token_count = int(motion_token_count)
            if not 0 < motion_token_count < x.shape[1]:
                raise ValueError(
                    "motion_token_count must split motion and text tokens, "
                    f"got {motion_token_count} for total {x.shape[1]}"
                )
            motion_h = h[:, :motion_token_count]
            text_h = h[:, motion_token_count:]
            motion_valid = valid[:, :motion_token_count]
            text_valid = valid[:, motion_token_count:]
            motion_pos = pos_ids[:, :motion_token_count]
            text_pos = pos_ids[:, motion_token_count:]
            motion_out = self.attn(
                motion_h,
                h,
                key_valid=valid,
                query_valid=motion_valid,
            ...
            attn_out = torch.cat([motion_out, text_out], dim=1)
        x = x + attn_mod.gate * attn_out
        h = self.norm2(x)
