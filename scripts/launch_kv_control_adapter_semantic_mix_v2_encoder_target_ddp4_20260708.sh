...
RUN=kv_control_adapter_semantic_mix_v2_encoder_target_ddp4_20260708
...
exec env CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}" PYTHONUNBUFFERED=1 \
  ...
  --kv_control_loss_weight 0.8 \
  --kv_control_loss_type l1 \
  --kv_control_profile semantic_mix \
  --kv_control_keyframe_strategy mixed \
  --kv_control_min_keyframes 1 \
  --kv_control_max_keyframes 8 \
  --kv_control_min_joints 1 \
  --kv_control_max_joints 6 \
  --kv_control_dropout_prob 0.05 \
