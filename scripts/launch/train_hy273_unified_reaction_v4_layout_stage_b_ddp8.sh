...
CONFIG="${CONFIG:-configs/hy273_unified_fulltext_reaction_v4_layout.yaml}"
...
RUN_NAME="${RUN_NAME:-hy273_unified_reaction_v4_layout_$(date +%Y%m%d_%H%M%S)}"
...
[[ "${NPROC_PER_NODE}" == "8" ]] || {
  echo "Reaction-v4 layout Stage B requires 8-card DDP" >&2
  exit 2
}
[[ "${STOP_STEP}" == "150000" || "${STOP_STEP}" == "200000" ]] || {
  echo "Reaction-v4 layout STOP_STEP must be 150000 or 200000" >&2
  exit 2
}
[[ -f "${CHECKPOINT}" ]] || {
  echo "Missing Reaction-v4 layout checkpoint: ${CHECKPOINT}" >&2
  exit 2
}
...
if checkpoint.get("format") != CHECKPOINT_FORMAT:
    raise RuntimeError("Reaction-v4 layout requires a unified-actor checkpoint")
step = int(checkpoint.get("next_global_step", -1))
if step < 100_000:
    raise RuntimeError(f"Reaction-v4 layout requires step >=100K, got {step}")
if step > 100_000 and checkpoint.get("run_name") != sys.argv[2]:
    raise RuntimeError("A Reaction-v4 continuation checkpoint belongs to another run")
print(step)
PY
)
...
echo "stage=reaction-v4-layout tasks=t2m,edit,reaction mix=30,35,35 run=${RUN_NAME} interval=${NEXT_STEP}:${STOP_STEP}"
EXTRA_ARGS=()
