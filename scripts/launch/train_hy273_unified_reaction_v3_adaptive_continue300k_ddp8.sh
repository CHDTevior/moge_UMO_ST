...
RUN_NAME="${RUN_NAME:-hy273_unified_reaction_v3_adaptive_20260804_0628_smoke}"
OUTPUT_DIR="${OUTPUT_DIR:-/mnt/afs/mogeflow-control/outputs/hy273_unified_reaction_v3_adaptive}"
RUN_DIR="${OUTPUT_DIR}/${RUN_NAME}"
CHECKPOINT="${CHECKPOINT:-${RUN_DIR}/model/latest.pt}"
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"
NPROC_PER_NODE="${NPROC_PER_NODE:-8}"
BASE_CONFIG="${BASE_CONFIG:-configs/hy273_unified_fulltext_reaction_v3_adaptive.yaml}"
EXTENDED_CONFIG="${EXTENDED_CONFIG:-configs/hy273_unified_fulltext_reaction_v3_adaptive_continue300k.yaml}"
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/mogo/bin/python}"

[[ -f "${CHECKPOINT}" ]] || {
  echo "Missing Reaction-v3 continuation checkpoint: ${CHECKPOINT}" >&2
  exit 2
}
[[ -d "${RUN_DIR}" ]] || {
  echo "Missing Reaction-v3 run directory: ${RUN_DIR}" >&2
  exit 2
}
[[ "${NPROC_PER_NODE}" == "8" ]] || {
  echo "Reaction-v3 continuation requires 8-card DDP" >&2
  exit 2
}
...
if (( CURRENT_STEP < 150000 || CURRENT_STEP > 300000 )); then
  echo "Expected a Reaction-v3 checkpoint in [150000,300000], got ${CURRENT_STEP}" >&2
  exit 2
fi
...
if (( CURRENT_STEP < 200000 )); then
  CONFIG="${BASE_CONFIG}" RUN_NAME="${RUN_NAME}" OUTPUT_DIR="${OUTPUT_DIR}" \
    ...
    NPROC_PER_NODE="${NPROC_PER_NODE}" MASTER_PORT=29813 \
    bash scripts/launch/train_hy273_unified_reaction_v3_adaptive_stage_b_ddp8.sh
...
if (( CURRENT_STEP < 250000 )); then
  CONFIG="${EXTENDED_CONFIG}" RUN_NAME="${RUN_NAME}" OUTPUT_DIR="${OUTPUT_DIR}" \
    ...
    NPROC_PER_NODE="${NPROC_PER_NODE}" MASTER_PORT=29814 \
...
if (( CURRENT_STEP < 300000 )); then
  CONFIG="${EXTENDED_CONFIG}" RUN_NAME="${RUN_NAME}" OUTPUT_DIR="${OUTPUT_DIR}" \
    ...
    NPROC_PER_NODE="${NPROC_PER_NODE}" MASTER_PORT=29815 \
    bash scripts/launch/train_hy273_unified_reaction_stage_b_continue50k_ddp8.sh
fi

echo "Reaction-v3 continuation reached 300K: ${RUN_DIR}/model/step_00300000.pt"
