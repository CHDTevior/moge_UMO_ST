...
RUN_NAME="${RUN_NAME:-hy273_r16_qwen_tokens_only_ddp8_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/outputs/hy273_multitask}"
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"

if [[ -e "${OUTPUT_DIR}/${RUN_NAME}" ]]; then
  echo "Fresh R16 pipeline refuses existing run directory: ${OUTPUT_DIR}/${RUN_NAME}" >&2
  exit 2
fi

export TEXT_GLOBAL_CONDITIONING=qwen_tokens_only
echo "R16 Qwen-tokens-only run=${RUN_NAME} stages=A:200K B:200K C1:50K C2:50K"
for stage in a b c1 c2; do
  ...
  STAGE="${stage}" RUN_NAME="${RUN_NAME}" OUTPUT_DIR="${OUTPUT_DIR}" GPU_IDS="${GPU_IDS}" \
    bash scripts/launch/train_hy273_multitask_r16_stage_ddp8.sh
