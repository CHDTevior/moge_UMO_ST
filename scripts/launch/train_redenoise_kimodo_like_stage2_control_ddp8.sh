...
CONFIG="${CONFIG:-configs/redenoise_kimodo_like_stage2_control.yaml}"
RUN_NAME="${RUN_NAME:-hy273_redenoise_kimodo_like_stage2_control_$(date +%Y%m%d_%H%M%S)}"
...
EXECUTION_CONTRACT="stage2_production"
LIMIT_ARGS=()
if [[ -n "${PILOT_MAX_STEPS:-}" ]]; then
  ...
  EXECUTION_CONTRACT="stage2_pilot"
...
require(
    cfg["control"]["modes"]
    == ["none", "root_sparse", "root_dense", "endpoints", "fullpose", "mixed"],
    "Stage-2 control modes do not match frozen v1",
)
require(float(cfg["loss"]["control_cont"]) > 0.0, "Stage-2 control loss must be positive")
require(float(cfg["loss"]["control_contact"]) == 0.0, "Stage-2 contact-control loss must be zero")
