...
CONFIG="${CONFIG:-configs/hy273_unified_fulltext_reaction_v3_adaptive.yaml}"
...
RUN_NAME="${RUN_NAME:-hy273_unified_reaction_v3_adaptive_$(date +%Y%m%d_%H%M%S)}"
CHECKPOINT="${CHECKPOINT:-${PARENT_CHECKPOINT}}"
STOP_STEP="${STOP_STEP:-150000}"
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"
NPROC_PER_NODE="${NPROC_PER_NODE:-8}"
MASTER_PORT="${MASTER_PORT:-29813}"
...
[[ "${NPROC_PER_NODE}" == "8" ]] || {
  echo "Reaction-v3 adaptive Stage B requires 8-card DDP" >&2
  exit 2
}
[[ "${STOP_STEP}" == "150000" || "${STOP_STEP}" == "200000" ]] || {
  echo "Reaction-v3 adaptive STOP_STEP must be 150000 or 200000" >&2
  exit 2
}
[[ -f "${CHECKPOINT}" ]] || {
  echo "Missing Reaction-v3 adaptive checkpoint: ${CHECKPOINT}" >&2
  exit 2
}
...
if checkpoint.get("format") != CHECKPOINT_FORMAT:
    raise RuntimeError("Reaction-v3 adaptive requires a unified-actor checkpoint")
step = int(checkpoint.get("next_global_step", -1))
if step < 100_000:
    raise RuntimeError(f"Reaction-v3 adaptive requires step >=100K, got {step}")
if step > 100_000 and checkpoint.get("run_name") != sys.argv[2]:
    raise RuntimeError("A Reaction-v3 continuation checkpoint belongs to another run")
print(step)
PY
)
...
echo "stage=reaction-v3-adaptive tasks=t2m,edit,reaction mix=30,35,35 run=${RUN_NAME} interval=${NEXT_STEP}:${STOP_STEP}"
EXTRA_ARGS=()
