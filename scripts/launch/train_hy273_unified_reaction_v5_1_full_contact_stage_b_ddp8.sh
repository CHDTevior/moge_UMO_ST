...
CONFIG="${CONFIG:-configs/hy273_unified_fulltext_reaction_v5_1_full_contact.yaml}"
...
RUN_NAME="${RUN_NAME:-hy273_unified_reaction_v5_1_full_contact_$(date +%Y%m%d_%H%M%S)}"
CHECKPOINT="${CHECKPOINT:-${PARENT_CHECKPOINT}}"
STOP_STEP="${STOP_STEP:-200000}"
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"
NPROC_PER_NODE="${NPROC_PER_NODE:-8}"
MASTER_PORT="${MASTER_PORT:-29819}"
...
[[ "${NPROC_PER_NODE}" == "8" ]] || {
  echo "Reaction-v5.1 Stage B requires 8-card DDP" >&2
  exit 2
}
[[ "${STOP_STEP}" == "200000" ]] || {
  echo "Reaction-v5.1 is designed to stop at 200000" >&2
  exit 2
}
[[ -f "${CHECKPOINT}" ]] || {
...
if checkpoint.get("format") != CHECKPOINT_FORMAT:
    raise RuntimeError("Reaction-v5.1 requires a unified-actor checkpoint")
step = int(checkpoint.get("next_global_step", -1))
if step < 100_000:
    raise RuntimeError(f"Reaction-v5.1 requires step >=100K, got {step}")
...
echo "stage=reaction-v5.1 tasks=t2m,edit,reaction mix=30,35,35 run=${RUN_NAME} interval=${NEXT_STEP}:${STOP_STEP}"
EXTRA_ARGS=()
